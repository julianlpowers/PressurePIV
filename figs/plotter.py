...
plt.plot(data['x'],data['Cp'],label='PIV + linear extrapolation from 0.02c')

data = np.load(r'AFOIL_streamlines_3%c_2%noise.npz')
plt.plot(data['x'],data['Cp'],label='PIV + streamline method from 0.02c')
...
plt.errorbar(theta_taps, Cp_taps, yerr=0.03, fmt='k.', mfc='none', barsabove=True, label='pressure taps', capsize=5)
...
plt.plot(data['theta'],data['Cp'],label='PIV + linear extrapolation from 0.04D')

data = np.load(r'CYLINDER_streamlines_2mm.npz')
plt.plot(data['theta'],data['Cp'],label='PIV + streamline method from 0.04D')

data = np.load(r'CYLINDER_raw_0mm.npz')
plt.plot(data['theta'],data['Cp'],label='PIV + all data used no extrapolation')
